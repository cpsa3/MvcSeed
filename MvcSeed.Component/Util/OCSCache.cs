namespace MvcSeed.Component.Util
{
    ...
    {
        readonly MemcachedClient MemClient = MemCached.getInstance();
        ...
        public T Get<T>(string key, Func<T> acquire, DateTime expiry, bool isRefreshForce = false, bool isNoPreFix = false)
        {
            ...
            LogHelper.Error(string.Format("ocskey:{0},isRefreshForce:{1},isNoPreFix:{2}", key, isRefreshForce, isNoPreFix));
            if (isRefreshForce)
            {
                T result = acquire();
    ...
    public sealed class MemCached
    {
        ...
        static void MemClientInit()
        {
            try
            {
                ...
                var ipEndPoint = new IPEndPoint(newAddress, int.Parse(ocsPort));
                ...
                memConfig.Authentication.Parameters["zone"] = string.Empty;
